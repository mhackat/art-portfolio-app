...
export async function GET() {
  ...
  try {
    const rows = (await prisma.$queryRawUnsafe(
      "SELECT current_database() as db, (SELECT count(*) FROM \"User\") as user_count, (SELECT count(*) FROM information_schema.columns WHERE table_name='User' AND column_name='failedLoginAttempts') as has_lockout_col"
    )) as any[];
